...
## Per-Deliverable Acceptance Criteria — S23 refinement (a′) DesignDecision count-grain pin + (b′) form-appropriate independent second reader for V (Cycle-16-S23 RP refinement; root fix of the S22 HOLD-AGAIN findings 1-2)
...
<!-- source: docs/spec_class_frozen_definition.md §7 (grain-pin) + ED §5.phase11.1-R-S23 threshold 6″/6″-FB/6″-EM + EXIT + HR §3.phase11.1-R-S23 rows (this same S23 RP fill) -->
<!-- source: cycle_16_s23_rp_refinement_dispatch_substrate §2 (grain top-down) + §3 (form-appropriate reader) + §4 (anti-gaming guards) + §5 (EXIT) -->
<!-- source: .claude/strategic_frame.md Binding 6 (RP authors, Coach does not) + Binding 7 (process-guided; no Coach-direct gate edit) + Binding 8 (HC-07/HC-11) -->
<!-- gate:requirements done_definition_phase11_refinement_s23 -->

**Scope LOCKED (four RQS spec classes ONLY; substrate §1; NOT reopened — kc-53 R2 operational disposition).** The denominator population is AgentContract + Schema + DesignDecision + MethodologyCommitment. `DP#N / HC#N / GPL / Pattern / Binding` are references, NOT denominator members. This is the RQS's own answer; it is NOT a Rex question and is NOT reopened. The S23 grain-pin operates INSIDE the DesignDecision class; it does not touch the population scope.

**Deliverable (a′) — the DesignDecision count-grain pin (RP authors this S23; the NEW freeze, committed FIRST).** Acceptance:
1. `docs/spec_class_frozen_definition.md` §7 pins the grain to **one decision-identity = one DesignDecision spec** (the ADR-ID / disposition decision-name, normalized), derived top-down from the `cycle16:DesignDecision` `rdfs:comment` (the atomic object is *a decision*) — NOT from the number of JSON sub-fields serializing a disposition, NOT from the number of recording sites. The dedup-across-recording-sites rule (mirror/citation ≠ a new spec) + the three sub-cases (mirror=1 / multi-field-block=1 / N-decision-block=N) + the per-method (M1/M2/M3) reconciliation mapping to decision-identities are all decidable by a scanner.
2. **Grain-pin freeze-before-count (the order is the whole point; substrate §4.1).** §7 is committed FIRST, as a NEW freeze, BEFORE the Build-Runner re-counts. The re-count (`outputs/denominator_dual_method.json`) cites the grain-pin by path + commit SHA; the cited commit is an ancestor of the re-count commit (`git merge-base --is-ancestor` exits 0). A re-count that does not cite a prior-committed grain-pin freeze is REFUSED.
3. **±5% must EMERGE, never tuned-to-collapse (substrate §4.2).** Under the pinned grain the three-method **decision-identity** spread is MEASURED. If the methods STILL diverge beyond ±5%, that is an HONEST FINDING, disclosed — the grain is NOT re-drawn / sub-case re-picked / normalization re-tuned to force agreement (a stable-because-tuned number is REFUSED). On residual divergence the cycle takes the disclosed-bound EXIT (criterion below), NOT a third counting refinement.

**Deliverable (b′) — the form-appropriate independent second reader for V (RP authors this S23 as threshold 6″; Build-Runner implements the code).** Acceptance:
1. ED §5.phase11.1-R-S23 threshold 6″ form-classifies each spec by prose fraction `p` (structured `p<0.50` → rule-based E1 as second reader; prose-heavy `p≥0.50` → a **diverse second LLM E1′**: different model from `claude-haiku-4-5` AND a genuinely different prompt, not E2's reworded, not E2-output-seeded). The 0.50 boundary is the principled prose-majority midpoint, NOT tuned to the current corpus.
2. **The floors are UNCHANGED and FIXED.** `T6P_CONTRIB_FLOOR=0.20 / T6P_SOLO_CEILING=0.80 / T6P_JACCARD_FLOOR=0.20` are NOT touched. Lowering any floor to make V pass IS the substitution recurring inside the guard and is REFUSED. Tuning E1′ to reproduce E2 IS the substitution recurring at the reader and is REFUSED.
3. **No shared blind spot (operational).** The second reader's solo (reader-only) contribution MUST be > 0 on its appropriate form (a strict subset of the first reader = reading inside its blind spot, REFUSED) AND the composition clears the UNCHANGED 6′ floors (each reader share ≥ 0.20, solo ≤ 0.80, aggregate Jaccard ≥ 0.20). `extraction_methods_distinct` is upgraded from the bare `method_e1 != method_e2` string-compare to assert mechanism-difference + (when both LLM) model AND prompt difference + no-seeding-from-first.
4. **Honest-not-yet is a valid output.** If, with a form-appropriate independent reader, V STILL fails 6″, that is the empirical finding that the corpus cannot be two-reader-validated to the principled floor — carried to the disclosed-bound EXIT, NOT papered over and NOT fixed by lowering a floor.
5. **Binding 7 boundary.** RP authors the CRITERION (this S23). The Build-Runner (next dispatch) implements the form-classifier + the diverse second LLM E1′ + the upgraded independence assert + the no-shared-blind-spot check in `scripts/spec_extraction_pipeline.py`; it does NOT touch the `T6P_*` floors. This is RP spec work reported to kc-53, NOT a Coach-direct gate edit.

**The disclosed-bound EXIT (substrate §5 — the authorized stopping rule; verbatim-faithful acceptance language).** After a principled a′ + b′ + the Build-Runner re-run + Coach re-verification, the outcome is EXACTLY ONE of:
- **Trusted** — ±5% emerges across methods AND V passes 6″ (incl. the UNCHANGED 6′ composition floors) → record the trusted denominator + V; the cycle proceeds to BE-S/BE-T/Phase 12.
- **Disclosed-bound (the honest, already-authorized form of the bar — NOT a retreat)** — if they still don't, that is the empirical finding that perfect cross-method agreement is not achievable for this corpus. Adopt the most-defensible disclosed-bound denominator (M1′ under the frozen §1 unit + the §7 pinned grain — it counts authoritative decision-identity definition sites directly) + the honestly-labeled extraction set, and PROCEED toward the gap list. The close claim becomes **"100% of the disclosed-bound discoverable population, residual R disclosed."** Do NOT loop a third counting refinement.
- **Exit guard (so disclosed-bound is honesty, not surrender — ALL THREE BIND):** (i) a **principled grain frozen BEFORE seeing whether it was convenient** (§7 committed first); (ii) the residual **explicitly bounded and SMALL after a′ does its work** — the exit covers the irreducible remainder, NOT the whole 63→424; a large or unexplained residual is **another HOLD, not an exit**; (iii) the residual **carried forward into every downstream "100%" claim** (BE-T gap list, #53, FINDINGS) — disclosed, never zeroed.

**BE-R S23 close-eval is PASS-all:** thresholds 1-5 + 6′ (composition, floors UNCHANGED) + 6″ (form-appropriate second reader + no shared blind spot) + 7/6″-FB (grain-pin freeze-before-count) + 8/6″-EM (±5%-emerge-or-disclosed-bound-exit). Coach R3 independently re-verifies after the Build-Runner re-run.

**Reality-vs-intent baseline (HR §3.phase11.1-R-S23).** At S22 the DesignDecision grain is unpinned (63→424 spread `[measured]`; DesignDecision = 190/268 raw `[measured]`) and V is single-reader-on-prose (e1_share 0.1177 `[measured]`; 1604 E2-only vs 95 E1-only). The S23 refinement (a′)+(b′) is the ROOT fix of both HOLD-AGAIN findings; the Build-Runner re-runs under it. The S22 V FAILS 6′ — the guard working, not a regression.

**HC-11 partition (Binding 8).** The grain-pin methodology (decision-identity grain + the three sub-cases + per-method reconciliation) + the form-classifier + the form-appropriate-reader independence rule + the principled floors = PUBLISHABLE. Per-spec decision-identities + per-spec V contents + per-spec provenance + the divergence/residual list = PIPELINE-IP-PRIVATE. **HC-07:** every number above traces to one source (`outputs/retroactive_scan_cycle_1_15_run.json` for the 190/268 + class distribution; `outputs/denominator_dual_method.json` for M1′=193 / dual-union=210 / 63→424; `outputs/validated_commitment_set.json` for v_size 1818 / e1_share 0.1177 / e2_solo_share 0.8823 / jaccard 0.0655).
