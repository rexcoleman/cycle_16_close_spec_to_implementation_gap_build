...
| Field | Value |
|---|---|
| **Artifact under acceptance** | BE-F 11-deliverable per ARTIFACT_CONTRACT §14.0: 4 probe primitives at `scripts/probes/{a,b,c,d}/probe_<class>.py` + 8 self-test fixtures at `tests/probes/fixtures/known_{good,bad}_{a,b,c,d}_*.json` + admission gate (Python module `__init__.py` + bash CLI wrapper `probe_library_admission.sh`) + Class C LLM-judge prompt template at `scripts/probes/c/llm_judge_prompt.md` + 3 NEW JSONL sinks (`probe_fire_events.jsonl` + `probe_library_self_test_events.jsonl` + `probe_library_admission_events.jsonl`) + §14 BE-F appends at 3 cycle_16 docs (Edit-per-section; LOCKED §0-§13 preserved) + DEPLOYMENT_LOG §2 row 6 + BUILD_DECISION_LOG §2 row 6 + state.json transition + DECISION_LOG D-S11-1 + `outputs/build_runner_envelope.yaml` OVERWRITE + `outputs/build_runner_events.jsonl` append with 1 `be_f_probe_library_ship.event` single-fire. NO govML touch at S11 (v2.8.5 STANDS UNMODIFIED; back-port → BE-I S14+ per HC #45 chain extension precedent). |
...
| 33 | ARTIFACT_CONTRACT §14.2 row 33 (admission gate refuses deliberately-broken probe; T12 negative test evidence) | `grep -c '"event_class": "probe_library_admission.refuse.event"' outputs/probe_library_admission_events.jsonl` ≥ 1 AND `ls scripts/probes/_broken_/ 2>/dev/null` empty post-T12 cleanup AND `bash scripts/probe_library_admission.sh && echo $?` returns 0 post-cleanup (canonical 4-probe admission clean) AND refuse event payload carries `payload.verdict: "refuse"` + `payload.self_test_exit_code: 1` + `payload.evidence` containing "FAIL: probe does not distinguish" string | At BE-F Step 8 T12 negative test + Step 13 admission re-scan post-cleanup | PASS / FAIL / FAILS-WITH-DIAGNOSED-SHAPE (KT-7 firing if T12 negative test does NOT trigger refusal) |
...
3. **Admission gate verdict** captured via `bash scripts/probe_library_admission.sh && echo $?` AND via `scripts/probes/__init__.py admit_all()` JSON return; both routes must agree.
...
| 3 | §14.1 row 33 (admission gate refuses broken probe; T12 evidence) | PASS | n/a | — | Cycle 16 BE-F | `grep -c '"event_class": "probe_library_admission.refuse.event"'` ≥ 1; post-T12 admission scan returns 0 |
...
| 5 | §14.1 row 35 (self-test events JSONL ≥8 + predicateType + per-class fixture pair) | PASS | n/a | — | Cycle 16 BE-F | `wc -l outputs/probe_library_self_test_events.jsonl` ≥ 8; predicateType + per-class fixture_class verified via Python aggregation |
