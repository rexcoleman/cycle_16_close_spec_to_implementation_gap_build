...
| Field | Value |
|---|---|
| **Artifact** | `~/cycle_16_close_spec_to_implementation_gap_build/scripts/probes/{a,b,c,d}/probe_<class>.py` (4 primitive files, executable, with `--self-test` + `--aggregate-cycle N` + `--spec-iri` modes) + `~/cycle_16_close_spec_to_implementation_gap_build/scripts/probes/__init__.py` (admission-gate Python module exposing `admit_all()` + `expose_admitted()` + ADMITTED_PROBES + REFUSED_PROBES) + `~/cycle_16_close_spec_to_implementation_gap_build/scripts/probe_mibrary_admission.sh` (bash CLI/CI wrapper; exit 0 iff all probes admitted; exit 1 on refusal) + `~/cycle_16_close_spec_to_implementation_gap_build/scripts/probes/c/llm_judge_prompt.md` (Class C LLM-judge contract template with HC #72 anti-substitution language explicit; forbids registry-text-only acceptance; calibration fixtures referenced) + `~/cycle_16_close_spec_to_implementation_gap_build/tests/probes/fixtures/known_{good,bad}_{a,b,c,d}_*.json` (8 fixtures; 1 known-good + 1 known-bad per class) + `~/cycle_16_close_spec_to_implementation_gap_build/outputs/probe_mibrary_self_test_events.jsonl` (≥8 self-test events at admission + 1 T12 deliberate-fail event = 9 rows) + `~/cycle_16_close_spec_to_implementation_gap_build/outputs/probe_mibrary_admission_events.jsonl` (5 events: 4 PASS + 1 REFUSE for T12 negative test) + `~/cycle_16_close_spec_to_implementation_gap_build/outputs/probe_fire_events.jsonl` (16 production-fire events; ≥3 per class; mix expected-implemented + expected-dormant per class) + §14 BE-F appends at 3 cycle_16 docs (Edit-per-section) + DEPLOYMENT_LOG §2 row 6 + BUILD_DECISION_LOG §2 row 6 + state.json transition + DECISION_LOG D-S11-1 + `outputs/build_runner_envelope.yaml` OVERWRITE + `outputs/build_runner_events.jsonl` append with 1 `be_f_probe_mibrary_ship.event` row |
...
| 33 | **`scripts/probes/{a,b,c,d}/` directory structure scaffolded** with empty class subdirs + `tests/probes/fixtures/` directory + 3 NEW empty JSONL sinks (`outputs/probe_mibrary_self_test_events.jsonl` + `outputs/probe_mibrary_admission_events.jsonl` + `outputs/probe_fire_events.jsonl`) writable at BE-F build step 2 | `find scripts/probes -type d` returns ≥5 dirs (probes + 4 class subdirs); `test -w outputs/probe_mibrary_self_test_events.jsonl` etc. succeed | halt-and-surface; sink-unwritable surfaces upstream filesystem misconfiguration |
...
| 24 | **Admission gate refuse-on-fail discipline** — `probe_mibrary_admission.sh` exits non-zero when ANY probe self-test does not distinguish known_good + known_bad; refusal events MUST emit to admission sink with `event_class: probe_mibrary_admission.refuse.event` (not silently dropped). | At every admission scan (CI invocation + at every BE close + at session-close gate consumption) | halt-and-surface; silent refusal contamination = registry-shape-bypass; admission gate predicate body itself becomes substitution-prone if refusal events are dropped |
| 25 | **Smoke-only fires NOT acceptance evidence per Done #15d strict** — `run_id` prefix `s11_be_f_production_*` is structural discriminator for acceptance fires; `run_id` prefix `s11_be_f_probe_mib_self_test_*` / `_smoke_*` / `_admission_*` are infrastructure fires NOT counted toward acceptance; R3 aggregation filters via `run_id` prefix. | At every R3 close-eval + at FINDINGS close + at session-close gate (BE-H structural-prevention piece (d) probe-coverage check inheritance) | halt-and-surface; smoke-only-as-acceptance is the failure shape Done #15d was authored to prevent; treat as KT-7 + KT-10 cross-firing surface |

### §14.4 BE-F Side Effects (diff vs §13)

| Surface | NEW effect at BE-F | Bounds |
|---|---|---|
| Filesystem writes | NEW: `scripts/probes/{a,b,c,d}/probe_<class>.py` (4 files; ~16-20KB each; executable) + `scripts/probes/c/llm_judge_prompt.md` (~5KB; HC #72 anti-substitution language explicit) + `scripts/probes/__init__.py` (~4KB; admission-gate module) + `scripts/probe_mibrary_admission.sh` (~3KB; bash CLI wrapper) + `tests/probes/fixtures/known_{good,bad}_{a,b,c,d}_*.json` (8 fixtures; ~1KB each) + 3 NEW JSONL sinks (`outputs/probe_mibrary_self_test_events.jsonl` + `outputs/probe_mibrary_admission_events.jsonl` + `outputs/probe_fire_events.jsonl`; 0B at scaffold; grow to ≥8 + ≥5 + 16 rows respectively at BE-F close); EDITS to 3 cycle_16 docs §14 BE-F appends (Edit-per-section; LOCKED §0-§13 preserved); EDITS to DEPLOYMENT_LOG §2 row 6 + BUILD_DECISION_LOG §2 row 6 (append-only); state.json transition + DECISION_LOG D-S11-1 fill; `outputs/build_runner_envelope.yaml` OVERWRITE (BE-E → BE-F envelope) ; `outputs/build_runner_events.jsonl` append (1 `be_f_probe_mibrary_ship.event` single-fire NEW event class at BE-F close + Cycle 15 BE#4 5-event baseline class fires) | ≤120KB cumulative new content at cycle_16/; no existing-file body modifications outside §14 appends + DEPLOYMENT/BUILD_DECISION row 6 append + envelope overwrite + state.json transition + DECISION_LOG append |
...
| Field | Value |
|---|---|
| **Version scheme** | PROBE_VERSION = "0.1" constant pinned in each of 4 probe bodies + PROBE_ADMISSION_LOCK_COMMIT pinned to admission-time SHA; library admission gate (`__init__.py` + `probe_mibrary_admission.sh`) PROBE_LIBRARY_VERSION = "0.1" pinned; modifications to probe bodies require version bump + new admission test run + Builder-ARCH paradigm dispatch (HC #74) |
...
| **Promotion gates** | BE-A/B/C/D/E gate stack inherited (`build_pipeline_gate.sh` + `production_deployment_gate.sh` + `cross_system_validation_gate.sh` + `hc26_internal_smoke_gate.sh` + `spec_implementation_present_gate.sh` + `spec_implementation_session_close_gate.sh`); BE-F ADDS `probe_mibrary_admission.sh` as build-time admission predicate AND the present-gate / session-close-gate predicate-body UPGRADE from registry-field SPARQL ASK to probe-fire-evidence aggregation is BE-H scope (Done #15 + Done #17 + H_recovery_7 + H_recovery_9), NOT BE-F. BE-F SHIPS the canonical vocabulary; BE-G/BE-H wires it into the existing gates. |
| **Rollback procedure** | If full BE-F rollback required: (1) `git -C ~/cycle_16_close_spec_to_implementation_gap_build checkout docs/{ARTIFACT_CONTRACT,RUNTIME_EMIT_SPEC,ACCEPTANCE_CRITERIA,DEPLOYMENT_LOG,BUILD_DECISION_LOG}.md DECISION_LOG.md state.json outputs/build_runner_envelope.yaml outputs/build_runner_events.jsonl` (revert §14 appends + state transition + envelope); (2) `rm -rf scripts/probes/ scripts/probe_mibrary_admission.sh tests/probes/fixtures/known_*.json outputs/probe_mibrary_self_test_events.jsonl outputs/probe_mibrary_admission_events.jsonl outputs/probe_fire_events.jsonl` (revert NEW artifacts); (3) NO govML revert required at BE-F (v2.8.5 STANDS UNMODIFIED — confirmed in §14.4 NONE at govML); (4) Cycle 6 LOCKED preserved throughout (verified at HC #26 Gate 3); (5) BE-A/B/C/D/E LOCKED bodies preserved throughout (verified at §14.1 row 31 git-diff invariant). Rollback NOT end-to-end tested at BE-F (defer to Cycle 17+ if Coach R3 disposition requires) |
...
- Smoke-vs-production discriminator semantics changes (`run_id` prefix convention `s11_be_f_production_*` vs `_smoke_*` vs `_probe_mib_self_test_*`) — CONTRACT_CHANGE required per §14.3 invariant 25 (Done #15d strict)
...
| 4 | Admission gate (`scripts/probe_mibrary_admission.sh`) returns exit 0 + emits 4 PASS events to `outputs/probe_mibrary_admission_events.jsonl` | [x] PASS (`bash scripts/probe_mibrary_admission.sh && echo $?` returns 0; `grep -c '"event_class": "probe_mibrary_admission.pass.event"' outputs/probe_mibrary_admission_events.jsonl` ≥ 4) |
| 5 | Admission gate REFUSES deliberately-broken probe (T12 negative test) — broken probe at `scripts/probes/_broken_/probe_broken.py` invoked via `probes._run_admission_for_probe()` returns ok=False + emits `event_class: probe_mibrary_admission.refuse.event`; broken probe REMOVED after T12 evidence captured | [x] PASS (T12 evidence captured at 2026-05-28: ok=False rc=1 evidence='self_test_exit=1 stderr_head=FAIL: probe does not distinguish known_good and known_bad fixtures'; `grep -c '"event_class": "probe_mibrary_admission.refuse.event"' outputs/probe_mibrary_admission_events.jsonl` = 1; `ls scripts/probes/_broken_/ 2>/dev/null` empty post-T12) |
...
| 7 | Self-test events JSONL ≥8 rows at `outputs/probe_mibrary_self_test_events.jsonl` with `predicateType: cycle16:probe_self_test_v1` (distinct from production `cycle16:probe_fire_v1`) | [x] PASS (`wc -l outputs/probe_mibrary_self_test_events.jsonl` ≥ 8 covering 4 probes × 2 fixtures + T12 row + Class C re-run row; predicateType field present in all rows) |
...
Auxiliary sinks: `outputs/probe_mibrary_auto_deprecate_events.jsonl` (>=1
`*.deprecate.event` when a probe fails self-test 2 consecutive sessions) +
`outputs/probe_mibrary_self_test_events.jsonl` (per-probe rows; `crash` bool).
...
## §16 BE-M — Probe-accuracy validation harness (Cycle-16-S17, Done #25/#38/#44)

**§16.0 Artifact identity**

| Field | Value |
|---|---|
| **Artifact name** | `probe_accuracy_harness` (`probe_accuracy_harness_v0.1`) |
| **Artifact kind** | validation harness (validate-the-validator: measures each Class A–F probe's accuracy vs independent execution-grounded ground truth) |
| **Repository path** | `~/cycle_16_close_spec_to_implementation_gap_build/scripts/probe_accuracy_harness.py` |
| **Invocation** | `python3 scripts/probe_accuracy_harness.py [--class {A,B,C,D,E,F,all}] [--scan-json PATH] [--sink PATH] [--summary PATH] [--query-endpoint URL] [--named-graph IRI] [--limit N] [--self-test]` |
| **Build cycle** | Cycle 16 (Stage 5 BE-M) |
| **Contract owner** | build Agent (BE-M authoring) → Coach R3 independent verify (Cycle-16-S17 close) |

**§16.1 Pre-conditions (MUST hold)**

- Population scan present: `outputs/retroactive_scan_cycle_1_15_run.json` (232 distinct specs).
- 6 probe CLIs present + runnable at `scripts/probes/{a,b,c,d,e,f}/`.
- Fuseki up at the SPARQL endpoint for B (SHACL conforming-instance count) and E (KG node read). If unreachable, B SHACL specs and E nodes degrade to `contested`/`unjoinable` (NOT a fabricated label).
- Transcript root readable for A: `/home/azureuser/.claude/projects/-home-azureuser-Moonshots-Career-Thesis-v2/`.

**§16.2 Post-conditions (GUARANTEED)**

- Per-class confusion matrix emitted (TP/FP/FN/TN/precision/recall/FP-rate/n + contested + deferred counts).
- `outputs/probe_accuracy_events.jsonl` (one `probe_accuracy.measure.event` row per spec + `probe_accuracy.summary.event` per class) and `outputs/probe_accuracy_summary.json` (per-class matrices + independence attestation) written.
- **Independence attestation TRUE**: the ground-truth deriver imports NO probe module (source-level: no `from ...probes...`/`import probe_`; runtime: no probe module in `sys.modules`). Probes invoked ONLY via subprocess CLI. `--self-test` exits 0 iff clean.
- NO `% implemented/faithful` claim about the codebase (Done #42). NO remediation. Honest PARTIAL/FAIL is a valid post-condition.

**§16.3 Outward-facing API**

- `--self-test` → prints the independence attestation JSON; exit 0 iff `independence_clean`.
- `--class X` → measures one class; `--class all` (default) → all six.
- `--sink` / `--summary` → relocate outputs. The harness sink is excluded from every ground-truth scan and is absent while any probe runs (anti-contamination; the probe-fire temp sinks live in `/tmp`, outside any `outputs/*.jsonl` glob a probe scans).
