...
| 2 | 2026-05-29T00:00:00Z | HOLD (NOT PROMOTED) — failing class: **B (CONTESTED)** + probe-A self-test FAIL | S18 detector remediation A/D/B re-validated via BE-M harness (run_id `s17_be_m_probe_accuracy_39d18a94`, independence_clean=true, harness/GT/fixtures UNEDITED). HOLD not PROMOTED because the PROMOTE condition (each execution-observable class hits its honest ceiling AND independence clean) is NOT met: D=FULL-RIGOR-PASS ✓ and A=FULL-RIGOR-PASS ✓ (harness awarded FULL-RIGOR not PASS-LOW-POWER: n_eval=8≥5 with pos+neg), but **B=CONTESTED — not validated to bar** (matrix itself clean TP6/FP0/FN0/TN1 recall 1.0, yet contested_frac=0.30>0.20 from 3 ontology .ttl files with no extractable sh:targetClass/owl:Class — B did NOT reach its PASS-LOW-POWER ceiling). Additionally probe-A per-probe `--self-test` FAILS (exit 1): known_good_a_1 asserts build-runner=True but GT labels build-runner contested/False under strict subagent_type dispatch (real dispatches use subagent_type='general-purpose'); the fixture encodes the now-removed OR-branch bug and CANNOT be satisfied by a GT-correct probe without re-introducing the bug or editing the fixture (both forbidden). Judged tier all NOT-VALIDATED: F-judge (live LLM at usage limit on primary AND alt key until 2026-06-01 → lone structural signal fails-safe), C judge (stubbed/deferred C17+), E status-match (imports BE-F independence defect). | build-runner (Cycle-16-S18) → Coach R3 re-verify → Rex paradigm ruling on the B-CONTESTED + A-fixture-conflict residuals | harness run_id `s17_be_m_probe_accuracy_39d18a94`; `outputs/probe_accuracy_summary.json`; `outputs/probe_accuracy_events.jsonl`; DEPLOYMENT_LOG §2 row 8 | N/A |
...
| 11 | BE-M | **Q2=C (build-class).** Ship probe-accuracy validation harness (Done #25/#38/#44 — validate-the-validator). Decision is the GROUND-TRUTH-DERIVER ARCHITECTURE. **Candidates (≥2): (a) independent re-implementation** — re-derive each label from the SAME external source the probe reads, with the harness's own code, never importing the probe; **(b) human expert labels** — a person hand-labels each spec; **(c) second LLM judge** — a different model labels implementation status. **Selected: (a) independent re-implementation.** Rationale: (b) human-label is RETIRED per Done #44 (autonomy floor — no mandatory human step in the validation path) and does not scale to 232; (c) second-LLM is GAP-2-BLOCKED this session (genuine Anthropic usage cap, Coach-confirmed Finding 1) AND re-introduces a non-deterministic oracle. (a) gives execution-grounded, deterministic, reproducible labels with the lowest correlated-error risk PROVIDED the deriver shares NO code with the probe — enforced structurally by `--self-test` (asserts the harness namespace imports no probe module; probes invoked via subprocess CLI only). Where (a) is impossible without the LLM (Class C live judge, Class F judge-fallback, Class E status-match which imports BE-F), the class is marked DEFERRED-GAP-2 rather than given a fabricated number (DP#44 refuse-on-missing-precondition). Where the ground-truth DEFINITION itself is contested (Class A subagent_type-dispatch vs description-mention per S141 separate-session caveat; Class D generic R-tokens), the harness pins the most defensible definition, reports the matrix under it, AND discloses the definition-sensitive count; >20% definition-sensitive → "contested, not a number" (Class B). Reversibility: HIGH (new file + new outputs only; no probe/gate/ontology touched; additive artifact rows). Compat: probes run unchanged via their existing `--aggregate-cycle` CLI. Carry-forward honest gaps: (i) A/B small-n (<20 unmeetable); (ii) E source/field sub-checks have NO negative cases on the real population → no FP-discrimination power proven (known-bad covered only by the probe's own self-test fixture, forbidden as accuracy GT); (iii) F n_eval=2 (264/269 DP#26 carve-outs) → PASS-LOW-POWER not a credible population PASS; (iv) C + E-status-match + F-judge-fallback DEFERRED-GAP-2 until Anthropic billing restored (Rex infra). | S17 |

(Stage 5 BE-M section 2 row 11 ADDITIVE per Cycle-16-S17; rows 1-10 LOCKED.)

| 12 | BE-O | **Q1=A.** Rewrite the Class A ground-truth DEFINITION + probe to the BEHAVIORAL-EXECUTION-OBSERVABLE per Rex ruling D-S18-1 (2026-05-29, commit 10dd363). DECISION: reject `subagent_type`-dispatch as the observable entirely. Rationale: S141 GUARANTEES it false — pipeline agents run as SEPARATE Claude Code sessions, never as Agent-tool subagents, so `subagent_type==<agent-name>` almost never appears; the old GT was a proxy measuring the wrong thing. SELECTED observable = the spec's committed `runtime_emit_event_class` actually EMITTED in `outputs/*_events.jsonl` (the Class-F execution-path observable applied to AgentContracts; KT-8 read the emission RECORD, not the registry). `n/a` committed → DP#26 carve-out (excluded). Authorized GT-DEFINITION change (the old def was WRONG, not a correct def weakened) — NOT a GT-value/fixture fudge. VERDICT: **PROMOTED (honest tier ceiling).** Probe + `gt_class_a` implement the SAME observable via SEPARATE code paths (independence_clean=true). Fixtures relabelled to behavioral truth (run-to-see). MEASURED: build-runner=implemented (class fires); implementation-coach + kernel-coach = committed-but-never-emitted = the REAL gap; 6/9 `n/a` carve-outs. Harness A = FULL-RIGOR-PASS-LOW-POWER (TP1/FP0/FN0/TN2 recall1.0, n_eval=3) — the honest finding is that most AgentContracts have NO executable behavioral observable, NOT a number engineered to pass. | S19 |

(Stage 5 BE-O section 2 row 12 ADDITIVE per Cycle-16-S19; rows 1-11 LOCKED.)

| 13 | BE-P | **Q2=C (build-class).** Make the JUDGED tier (C live judge / E status-match / F judge-path) trustworthy via diverse, structurally-independent AGREEING automated judges (Discipline #30) + build the harness independent GT derivers using a DIFFERENT model than the probe judge (validate-the-validator #19). DECISION on the GT-judge independence mechanism — candidates: (a) same model both sides (REJECTED: a judge agreeing with itself is not independence); (b) DIFFERENT model + DIFFERENT cross-check method (SELECTED: probe judge=claude-haiku-4-5, GT judge=claude-sonnet-4-6 + harness's own code-token grep as the structural cross-check); (c) human GT (REJECTED per Done #44 autonomy floor). GAP-2 RESOLVED (Rex: usage cap lifted; .env.backup key; live haiku+sonnet verified). C probe live judge un-stubbed (agreement-required with `_structural_judge`); E `_status_match` DECOUPLED from importing BE-F (was the #19/#25 defect — validator importing the thing it validates) → independent emission-record re-derivation; F judge-fallback exercised live. GT derivers `gt_class_c` (BLIND random 20-sample of 190, seed=1616, ids disclosed), `gt_class_e_status`, `gt_class_f_judge`. VERDICT: **HOLD (judged tier NOT validated — honest not-yet).** C = CONTESTED (blind-20: 11 agree-TN, 6 genuine judge↔crosscheck disagreement, 3 unparseable; contested_frac 0.45>0.20 → conservative fail-safe). E status-match = CONTESTED (4 agreeing eval pairs recall1.0 but 228/232 not independently derivable). F judge-path = CONTESTED (n_eval=1). Root cause: diverse judges (haiku vs sonnet) + structural cross-check genuinely do NOT robustly agree across the population, and embodiment-resolution for DesignDecision specs is heuristic → much "embodiment_not_found" / ambiguity. This is the CORRECT validate-the-validator outcome: the judged tier is held at CONTESTED rather than force-promoted. NO GT/fixture fudge; independence_clean=true. NOT promoted to the execution bar (tier-disclosed per Done #51). | S19 |

(Stage 5 BE-P section 2 row 13 ADDITIVE per Cycle-16-S19; rows 1-12 LOCKED. HOLD-CONTESTED on the judged tier is the honest fail-safe, not a defect to hide.)
