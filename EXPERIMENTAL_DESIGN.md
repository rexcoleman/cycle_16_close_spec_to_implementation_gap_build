...
## §5.phase11 Stage 5 Acceptance Criteria — BE-R + BE-S + BE-T (Cycle-16-S20 Phase-11 RP fill per SI Amendment 2026-05-29g (28g) ruling (a); Done #49/#50/#51 + guard-the-guards #47; Phase 11 FINDS + CERTIFIES gaps, does NOT implement missing code)

<!-- source: SI Amendment 2026-05-29g (28g) Done #49/#50/#51 + ruling (a) verbatim -->
<!-- source: cycle_16_s20_phase11_rp_dispatch_substrate §2 falsifiable acceptance bars + §3.2 ED authoring scope -->
<!-- source: HR §3.phase11.1 H_phase11_49/50/51 + §3.phase11.2 reality-vs-intent rows (this same S20 RP fill) -->
<!-- source: scripts/probe_accuracy_harness.py load_population + gt_class_c/e_status/f_judge + independence_attestation (--self-test); outputs/retroactive_scan_cycle_1_15_run.json enumeration_methodology -->
<!-- source: handoff §1ee — execution tier trustworthy to honest ceilings; judgment tier CONTESTED (honest not-yet) -->
<!-- gate:experimental_design §5.phase11 stage_5_phase11_acceptance required -->

**Discipline.** Phase 11 finds and certifies the spec→implementation gaps; it does NOT implement the missing spec code (Phase 12 verify → Phase 13 build/kill/defer). Each BE-N threshold below is an INDEPENDENCE-CHECK-grounded or VALIDATED-SET-grounded bar a Build-Runner can FAIL — never a string-match / single-reader / single-judge / engineered-agreement proxy. Per dispatch §5 the four recursive-failure moves are REFUSED structurally: (i) denominator dual-scan of recorded artifacts presented as two methods; (ii) detector run on a single-reader extraction; (iii) judge agreement engineered or a human in the close path; (iv) any "100%" not scoped to the discoverable population with residual stated. Whatever implemented-rate the eventual trusted-detector run yields IS the finding (it will likely show large gaps — e.g. the 6/9 AgentContracts with no executable observable per §1ee); never a number to engineer.

**Acceptance verdict pattern.** Each BE-N requires PASS-all on 6 acceptance thresholds (mirrors §5.1-5.5 + §5.7-5.10 `PASS-all (6 of 6)` precedent). Coach R3 independently re-verifies at BE close-eval per PD §3.3 T1-T13 mechanical tests. HC #72 substitution-detection self-scanned at every threshold draft before write; 0 substitution candidates retained.

**LOCKED-body discipline (Binding 7 + DP#42 strict).** §5.1-§5.recovery LOCKED at prior phase-checkpoint closes; §5.phase11 is ADDITIVE-APPEND only. Verify via `git diff` zero change on pre-marker line ranges (HC #72 LOCKED-body discipline).

### §5.phase11.1 BE-R Acceptance Criteria (Denominator honesty #50 + spec-extraction completeness #49; closes Done #50 + #49 + H_phase11_50 + H_phase11_49)

**Scope statement.** BE-R ships (i) the authored-intent scan (Method 2) over sources DISJOINT from Method 1 + the `methods_read_disjoint_sources` independence assertion + the `authored_but_unrecorded` divergence finding + the disclosed residual R; and (ii) the second-independent-extraction pipeline (E1 rule-based + E2 different-mechanism) + per-spec/aggregate Jaccard completeness + the `E1 △ E2` adjudication ledger + the reconciled validated set V that becomes detector input. Phase 11 sequencing: denominator (#50) → extraction (#49); the trusted-detector run (BE-T) consumes V.

**PASS-all (6 of 6) acceptance thresholds (independence-check / validated-set grounded; dual-scan + single-reader REFUSED):**

1. **Method 2 exists + reads disjoint sources (the #50 highest-risk trap, structural).** Authored-intent enumerator at `scripts/authored_intent_scan.py` enumerates specs from git-history of spec-bearing commits + DECISION_LOG/handoff/SI-amendment prose + session transcripts; `methods_read_disjoint_sources(S1_source_paths, S2_source_paths)` returns True where S1 = the 4 `enumeration_methodology` recorded sources, S2 = {git object history, prose-ruling bodies, transcript streams}; assertion HARD-FAILS (exit non-zero) if S2 ∩ S1 ≠ ∅ (a dual-scan of recorded artifacts is REFUSED). Verify via `python3 scripts/authored_intent_scan.py --independence-check` exits 0 only when source sets are provably disjoint; emits `event_class: denominator_independence_check.fire.event` with `disjoint_bool: true`.
2. **Divergence surfaced as a finding with per-spec provenance.** `authored_but_unrecorded = |S2_specs ∖ S1_specs|` [measured] emitted to `outputs/denominator_dual_method.json` with a per-spec provenance row (the commit SHA / DECISION_LOG ADR / transcript marker that authored it + the absence-of-record evidence that it is NOT in the 232 recorded set). Verify via JSON schema check: each `authored_but_unrecorded` entry has `provenance_source` + `recorded_set_absence_evidence` non-empty.
3. **Residual R bounded + disclosed; never rounded to zero.** `outputs/denominator_dual_method.json` carries `residual_R` block stating R = intentions written nowhere discoverable by either method, disclosed as "irreducible blind spot of BOTH methods, NOT estimated to zero" with the bounding procedure stated verbatim; close claim string is `"100% of the discoverable population (recorded ∪ authored-intent), residual R disclosed"`. Verify: `grep -c '"residual_R"' outputs/denominator_dual_method.json` ≥ 1 AND no bare `"100%"` token appears in any close surface without the scoping clause (mechanical scan).
4. **Second extraction exists + uses a distinct mechanism (the #49 highest-risk trap, structural).** `scripts/spec_extraction_pipeline.py` builds E1 (rule-based must-clause/SHALL/MUST parser) and E2 (independent-mechanism extraction at a DIFFERENT model/reader); `extraction_methods_distinct(method_E1, method_E2)` returns True (mechanism identifiers differ); assertion HARD-FAILS if E1 and E2 share a mechanism (single reader twice is REFUSED). Verify via `python3 scripts/spec_extraction_pipeline.py --methods-distinct-check` exits 0 only when distinct; emits `event_class: extraction_methods_distinct_check.fire.event`.
5. **Completeness measured + disclosed; symmetric difference surfaced not dropped.** Per-spec + aggregate `jaccard = |E1∩E2|/|E1∪E2|` [measured] emitted to `outputs/extraction_completeness.json`; every member of `E1 △ E2` written to an adjudication ledger (which reader missed it + why) — NOT discarded. Verify: aggregate Jaccard present + per-spec rows ≥ population count + `symmetric_difference_ledger` length == `|E1 △ E2|` (no silent drops; mechanical count equality).
6. **Detector input = the reconciled validated set V; single-reader extraction REFUSED.** The reconciled set V (= E1∩E2 + adjudicated E1△E2 members) is materialized at `outputs/validated_commitment_set.json` with `detector_input_is_reconciled_validated_set: true`; a guard (`scripts/spec_extraction_pipeline.py --assert-detector-input`) HARD-FAILS if any detector is wired to consume E1-alone or E2-alone (un-reconciled single-reader extraction). Verify: guard exits non-zero on a negative fixture wiring a single-reader extraction as detector input; exits 0 on V.

**HC-11 partition.** Method-2 scanner + extraction pipeline + independence-check assertions + completeness metric code = PUBLISHABLE (methodology). Per-spec authored-intent provenance + commitment-set contents + divergence list = PIPELINE-IP-PRIVATE (internal cycle spec content).

**Close evidence file paths.** `scripts/authored_intent_scan.py` + `scripts/spec_extraction_pipeline.py` + `outputs/denominator_dual_method.json` (Method-1 232 [measured] + Method-2 count + `authored_but_unrecorded` with per-spec provenance + `residual_R`) + `outputs/extraction_completeness.json` (per-spec + aggregate Jaccard + `symmetric_difference_ledger`) + `outputs/validated_commitment_set.json` (reconciled V) + `outputs/denominator_independence_check.jsonl` + `outputs/extraction_methods_distinct_check.jsonl` + DECISION_LOG entry `D-S{N}-{M}` at BE-R close.

**KT cross-binding.** Substitution refusal is structural: threshold 1 (dual-scan REFUSED) + threshold 4 (single-reader-twice REFUSED) + threshold 6 (single-reader-as-detector-input REFUSED) are the #50/#49 traps encoded as HARD-FAIL assertions. Reality-vs-intent baseline (HR §3.phase11.2): both Method 2 and the second extraction DO NOT EXIST today — BE-R builds them; this is the gap, not a regression.

### §5.phase11.2 BE-S Acceptance Criteria (Validation-tier close-path verdict mechanism #51; closes Done #51 + H_phase11_51)

**Scope statement.** BE-S productionizes the S19 diverse-agreeing-judge + tier-disclosure machinery (`gt_class_c`/`_status`/`_f_judge` in `probe_accuracy_harness.py`) into a per-spec CLOSE-PATH VERDICT function: execution-checkable specs → full-rigor verdict; semantically-judged specs → diverse-agreeing-judge verdict with fail-safe; per-spec tier label in the completion claim; NO human in the close path (ruling (a)). It does NOT re-build the judges (they exist); it wires them into the close-path verdict + proves the close path takes 0 human inputs + emits the tier-labelled claim. **Do NOT engineer agreement; do NOT force-certify the CONTESTED judgment tier** — it resolves via genuine judge convergence OR honest fail-safe NOT-VALIDATED.

**PASS-all (6 of 6) acceptance thresholds:**

1. **Per-spec tier classifier.** `scripts/validation_tier_verdict.py` classifies each spec execution-checkable vs semantically-judged and emits the per-spec tier label to `outputs/validation_tier_verdicts.json`; verify every spec in the validated set V has a `tier` field ∈ {execution_checkable, semantically_judged}.
2. **Execution tier = full rigor OR honest LOW-POWER (no silent upgrade).** Execution-tier verdict = `(FP==0 AND recall≥0.90)` → VALIDATED-FULL-RIGOR, OR explicitly `PASS-LOW-POWER` with `n_eval` disclosed (per §1ee: D r=1.0 FULL-RIGOR; A/F PASS-LOW-POWER n_eval=3/n_eval=2; E src/field FULL-RIGOR). Verify: no execution-tier spec is labelled VALIDATED-FULL-RIGOR without `FP==0 AND recall≥0.90` in its evidence row (mechanical assertion against the §1ee matrix re-fire).
3. **Judgment tier requires diverse judges (different model/method) — else REFUSED (the #51 highest-risk trap, structural).** Judgment-tier verdict consults ≥2 judges where `judge_uses_different_model_or_method(judge_GT, judge_probe)` returns True (GT=`claude-sonnet-4-6` ≠ probe=`claude-haiku-4-5`; harness `_gt_code_token_crosscheck` is a DIFFERENT code path from the LLM read); assertion HARD-FAILS if the two judges share model AND method (a judge agreeing with itself is REFUSED). Verify via `python3 scripts/validation_tier_verdict.py --judge-diversity-check` exits 0 only when diverse; emits `event_class: judge_diversity_check.fire.event` with `diverse_bool: true`.
4. **Fail-safe on disagreement/ambiguity/no-signal → NOT-VALIDATED (never engineered agreement).** Disagreement / unparseable / no-key / missing-signal → conservative NOT-VALIDATED, surfaced for after-the-fact human audit. Verify: a negative fixture with two disagreeing judge verdicts yields NOT-VALIDATED (NOT a coerced agreement); the S19 ~45% [measured] C-disagreement reproduces as CONTESTED → NOT-VALIDATED (the fail-safe firing is the bar working, not a defect). Reproduce: `gt_class_c` on the blind-20 sample yields the contested rate; the verdict function does NOT certify those specs.
5. **No human in the close path (ruling (a), structural).** The close-path verdict function `validation_tier_verdict.py::close_verdict()` takes NO human input parameter; `close_verdict_fn_human_inputs == 0` asserted mechanically (signature inspection: no interactive prompt, no Rex-disposition argument, no manual-override flag). Human audit is strictly after-the-fact (a separate read-only surface). Verify via `inspect.signature(close_verdict)` showing 0 human-input parameters + a negative fixture attempting a manual-override is REFUSED.
6. **Tier-labelled completion claim; uniform "100% validated" REFUSED.** The completion claim at `outputs/validation_tier_verdicts.json` declares per-spec which tier validated it + discloses the judgment-tier residual confidence gap (the CONTESTED specs are NOT counted as validated); a uniform "100% validated" spanning tiers HARD-FAILS a mechanical scan (`grep` for an un-tier-scoped "100% validated" string → gate fail). Verify: the claim string is two-tier + residual-disclosed; no un-scoped "100% validated" token present.

**HC-11 partition.** Tier classifier + verdict function + diversity-check + fail-safe logic = PUBLISHABLE (methodology; the diverse-judge discipline is the publishable contribution). Per-spec judge verdicts + judged-tier contested list + spec content = PIPELINE-IP-PRIVATE.

**Close evidence file paths.** `scripts/validation_tier_verdict.py` (per-spec close-path verdict; 0 human inputs) + `outputs/validation_tier_verdicts.json` (per-spec tier label + execution/judgment verdict + residual disclosure) + `outputs/judge_diversity_check.jsonl` + `outputs/validation_tier_failsafe_events.jsonl` (NOT-VALIDATED on disagreement) + reference to `scripts/probe_accuracy_harness.py` `gt_class_c`/`gt_class_e_status`/`gt_class_f_judge` + `--self-test` independence attestation (UNMODIFIED; consumed not rebuilt) + DECISION_LOG entry `D-S{N}-{M}` at BE-S close.

**KT cross-binding.** Substitution refusal is structural: threshold 3 (same-model/same-method judge REFUSED) + threshold 4 (engineered agreement REFUSED, fail-safe mandatory) + threshold 5 (human-in-close-path REFUSED) are the #51 + ruling-(a) traps encoded as HARD-FAIL assertions. Force-certifying the CONTESTED judgment tier (honestly not-yet per §1ee) is REFUSED — it resolves via genuine convergence or honest fail-safe.

### §5.phase11.3 BE-T Acceptance Criteria (Guard-the-guards #47 + trusted-detector run over the validated set; closes Done #47 + consumes BE-R V + BE-S verdicts)

**Scope statement.** BE-T (i) folds the enforcement infrastructure itself — probes, gates, agent specs, AND the accuracy harness — INTO the audited population (an unaudited guard is the recursive failure in miniature); and (ii) runs the trusted execution-tier detectors over the reconciled validated commitment set V (NOT a single-reader extraction), tier-labelling each verdict per BE-S. Sequencing: guard-the-guards folds infra into the denominator BEFORE the detector run; the run consumes BE-R's V + BE-S's tier verdicts. **Whatever implemented-rate it yields IS the finding** (likely large gaps); it is never a number to engineer, and the CONTESTED judgment tier is NOT force-certified.

**PASS-all (6 of 6) acceptance thresholds:**

1. **Enforcement infra enumerated INTO the audited population (#47, the guard-the-guards bar).** The probes (`scripts/probes/{a,b,c,d,e,f}/*.py`), the gates (`scripts/*_gate.sh` + `~/ml-governance-templates/scripts/*gate*.sh`), the agent specs (per `prompt_inventory` active rows), AND the accuracy harness (`scripts/probe_accuracy_harness.py`) are added as audited entries to the denominator at `outputs/guard_the_guards_population.json`; verify the audited-population count = denominator + |infra entries| and each infra entry carries a `is_enforcement_infra: true` flag. An infra surface absent from the audited population HARD-FAILS this threshold.
2. **Detectors run over the infra entries too.** The trusted execution-tier detectors fire over the `is_enforcement_infra: true` entries (the guards audit themselves); verify ≥1 detector-fire JSONL row per infra class present in `outputs/guard_the_guards_run.jsonl` (the harness's own implementation status is measured, not assumed).
3. **Detector input is the reconciled validated set V (single-reader REFUSED).** The detector run consumes `outputs/validated_commitment_set.json` (BE-R V); a guard HARD-FAILS if the run is wired to the raw single-reader BE-D scan instead of V. Verify: run manifest `detector_input_path` == validated_commitment_set.json + `detector_input_is_reconciled_validated_set: true`.
4. **Per-spec tier-labelled implemented-rate emitted (the finding).** `outputs/trusted_detector_run.json` reports, per spec, implemented / not-implemented / contested with its validation tier (per BE-S); the aggregate implemented-rate is [measured] and tier-scoped (execution tier separately from judgment tier; CONTESTED specs NOT counted as validated). Verify: aggregate rate present + tier-partitioned + no judgment-tier CONTESTED spec counted as implemented.
5. **Independence preserved at run-time (validate-the-validator).** The run re-asserts `probe_accuracy_harness.py --self-test` `independence_clean == true` (the harness imports NO probe; probes invoked via subprocess only) BEFORE accepting the run; verify `outputs/trusted_detector_run.json` carries the independence attestation with `independence_clean: true`. If independence is not clean, the run is REFUSED (the prober would be its own proxy).
6. **No "100%" not scoped; gaps disclosed honestly (HC #70).** Any "100%" in BE-T close evidence is scoped to the discoverable population with residual R stated (from BE-R) and tier-labelled (from BE-S); the honest gap (e.g. 6/9 AgentContracts with no executable observable per §1ee, the 3/10 KG ontologies with no SHACL shapes, the 228/232 E-status not independently re-derivable) is disclosed verbatim, NOT rounded away. Verify: mechanical scan finds no un-scoped "100%" + the §1ee-class honest gaps appear in the close evidence.

**HC-11 partition.** Guard-the-guards population builder + trusted-detector run harness invocation + independence attestation = PUBLISHABLE. Per-spec implemented-rate + infra-audit verdicts + contested list = PIPELINE-IP-PRIVATE.

**Close evidence file paths.** `outputs/guard_the_guards_population.json` (denominator + enumerated infra entries `is_enforcement_infra: true`) + `outputs/guard_the_guards_run.jsonl` (detectors fire over infra) + `outputs/trusted_detector_run.json` (per-spec tier-labelled implemented-rate over V + independence attestation `independence_clean: true`) + reference to `scripts/probe_accuracy_harness.py --self-test` (UNMODIFIED) + DECISION_LOG entry `D-S{N}-{M}` at BE-T close.

**KT cross-binding.** Threshold 3 (single-reader-as-input REFUSED — inherits BE-R #49 trap) + threshold 5 (independence preserved — validate-the-validator) + threshold 6 (no un-scoped "100%", gaps disclosed — HC #70). The implemented-rate is the finding (likely large gaps); force-certifying it or the CONTESTED judgment tier is REFUSED.

### §5.phase11 aggregate ledger forward

| BE | Closing Done# | Closing H_phase11 | Substitution trap encoded as HARD-FAIL | HC-11 partition | Expected Cycle 16 session |
|---|---|---|---|---|---|
| BE-R | #50 + #49 | H_phase11_50 + H_phase11_49 | dual-scan-of-recorded REFUSED (T1) + single-reader-twice REFUSED (T4) + single-reader-as-detector-input REFUSED (T6) | scanners/pipeline/metrics = PUBLISHABLE; spec content + provenance = PIPELINE-IP-PRIVATE | Cycle-16-S21+ Stage 5 |
| BE-S | #51 | H_phase11_51 | same-model/method judge REFUSED (T3) + engineered-agreement REFUSED / fail-safe mandatory (T4) + human-in-close-path REFUSED (T5) | tier/verdict/diversity/fail-safe logic = PUBLISHABLE; per-spec judge verdicts = PIPELINE-IP-PRIVATE | Cycle-16-S22+ Stage 5 |
| BE-T | #47 | (audited-population-membership requirement; consumes BE-R V + BE-S verdicts) | single-reader-as-input REFUSED (T3) + independence preserved (T5) + no un-scoped "100%" (T6) | population builder + run harness = PUBLISHABLE; per-spec rate = PIPELINE-IP-PRIVATE | Cycle-16-S23+ Stage 5 |

**Aggregate decision rule for §5.phase11.** Each BE-N requires PASS-all on 6 thresholds at Coach R3 close-eval. Sequencing: BE-R denominator (#50) → BE-R extraction (#49) → BE-T guard-the-guards folds infra in → BE-T trusted-detector run over V; BE-S validation-tier verdict binds from BE-S onward and tier-labels the BE-T run. The implemented-rate from BE-T IS the finding; large gaps are expected and disclosed (HC #70), never engineered. Route-forward: Phase 11 certifies the gaps → Phase 12 (100% probe-fire verification) → Phase 13 (build/kill/defer the missing code).

<!-- /gate:experimental_design §5.phase11 stage_5_phase11_acceptance -->

<!-- amendment_2026_05_29_phase11_end -->

<!-- amendment_2026_05_30_s22_refinement_start -->

### §5.phase11.1-R S22 refinement — threshold 6′ (V composition) + the freeze-before-count binding (Cycle-16-S22 RP refinement (a)+(c); supersedes-per-DP#42-non-destructive the label-only threshold 6)

<!-- source: S21 Coach R3 HOLD findings 1-3 (cycle_16 01a165d, ADR-S21-1, BUILD_DECISION_LOG row 14-R3) -->
<!-- source: docs/spec_class_frozen_definition.md (the freeze-before-count canonical artifact authored this same S22 RP fill) -->
<!-- source: cycle_16_s22_rp_refinement_dispatch_substrate §3 (anti-gaming guards) + §5 (strengthened T6) -->
<!-- source: outputs/extraction_completeness.json S21 run (aggregate_jaccard 0.0134, ∩22/∪1638) + outputs/validated_commitment_set.json S21 run (v_size 1637; E1∩E2=21 / E2_only=1598 / E1_only=18) [measured] -->
<!-- gate:experimental_design §5.phase11.1-R stage_5_phase11_refinement required -->

**LOCKED-body discipline (Binding 7 + DP#42 strict).** §5.1-§5.recovery and §5.phase11 (incl. the original threshold 6) are LOCKED; this §5.phase11.1-R is ADDITIVE-APPEND only. Threshold 6′ below **supersedes (DP#42 non-destructive) the original threshold 6** for BE-R close-eval — the original threshold 6 text is UNMODIFIED in place; 6′ adds the composition check 6 lacked. Verify via `git diff` zero change on all pre-marker line ranges (HC #72).

**Why 6′ exists (the S21 HOLD).** The original threshold 6 + the `--assert-detector-input` guard check V's **label/path** (`detector_input_is_reconciled_validated_set: true` + filename == `validated_commitment_set.json`). At S21 that PASSED while V was **97.6% one reader** [measured]: aggregate Jaccard 0.0134 (∩22 / ∪1638); V composition v_size 1637 = E1∩E2 21 (1.3%) + adjudicated:E2_only 1598 (97.6%) + adjudicated:E1_only 18 (1.1%). E1 (the rule-based reader) is effectively inert — its solo contribution (E1∩E2 ∪ E1_only) is 39/1637 = 2.4% [measured]. The label-only guard cannot see this. 6′ checks **composition**, not label.

**Threshold 6′ (strengthened; replaces 6 for BE-R close — per-reader contribution floor + non-trivial intersection floor).** The reconciled validated set V is a genuine TWO-READER set, asserted mechanically over V's composition:

- **Per-reader contribution floor (each reader materially contributes).** Define, over V, each reader's *contribution share* = (members the reader found, solo + shared) / |V|: `e1_share = |E1∩E2 ∪ E1_only| / |V|` and `e2_share = |E1∩E2 ∪ E2_only| / |V|`. **Each independent reader's contribution share MUST be ≥ 0.20**, AND **neither reader's solo (reader-only) share may exceed 0.80** (i.e. `|E1_only|/|V| ≤ 0.80` and `|E2_only|/|V| ≤ 0.80`). **Derivation (provably principled, NOT back-fitted):** "two-reader validation" *means* each reader is a load-bearing independent check; a reader contributing < 1/5 of the validated set is a garnish, not a validator. The 0.20 floor is `1/(number-of-readers + 3)` — a deliberately *lenient* principled minimum (a reader may legitimately be weaker, but not negligible); the symmetric 0.80 solo-dominance ceiling is its complement (1 − 0.20) so a single reader cannot supply the validated set under a thin co-reader garnish. The floor is derived from what "two readers" means, NOT set to whatever the current run passes — and indeed the S21 V (E1 share 2.4%) **FAILS** 6′ (2.4% < 20%), which is the criterion working.
- **Non-trivial intersection (Jaccard) floor.** Aggregate `jaccard = |E1∩E2| / |E1∪E2|` **MUST be ≥ 0.20**. **Derivation:** if two competent independent readers extract commitments from the SAME prose, their agreement on a non-trivial fraction is the evidence the extraction is reproducible across readers; a near-zero Jaccard (S21's 0.0134) means the two readers are not reading the same obligations at all — V is then a UNION of two disjoint single-reader extractions, not a reconciled two-reader set. 0.20 is the same principled "material agreement" floor (≥ 1/5 of the union is shared); it is NOT the S21 number (0.0134) nor any number the current run passes.
- **The guard HARD-FAILS when V is single-reader-dominated.** A V in the S21 state (0.0134 Jaccard / E1 share 2.4% / E2_only 97.6%) MUST FAIL the strengthened guard. **Negative fixture requirement:** an artificially single-reader-dominated V (e.g. E1 contributes < 20%, or aggregate Jaccard < 0.20) is REFUSED (guard exits non-zero); a genuinely two-reader V (both shares ≥ 0.20, Jaccard ≥ 0.20) PASSES. The Build-Runner authors both fixtures; the guard must reject the negative and accept the positive.
- **Independence preserved (E1 ≠ E2, validate-the-validator).** 6′ does NOT relax threshold 4 (`extraction_methods_distinct`): E1 stays rule-based, E2 stays LLM. The fix to the inert E1 (Build-Runner deliverable (b)) is "make E1 read the spec's actual definition-site prose COMPETENTLY," NOT "tune E1 to mimic E2." A 6′ PASS achieved by collapsing E1 into E2's mechanism is REFUSED (it would trip threshold 4).

**You re-author the CRITERION (spec) only.** The Build-Runner (deliverable (b), next dispatch) implements the composition check in `scripts/spec_extraction_pipeline.py::assert_detector_input` (extend it from label-check to composition-check) + the two fixtures. Per Binding 7 this is RP spec work reported to kc-53, NOT a Coach-direct gate edit.

**Threshold 7 (NEW; freeze-before-count binding — substrate §3.1).** The BE-R re-run (deliverable (b)) is bound to the frozen spec-class definition: (i) the canonical artifact `docs/spec_class_frozen_definition.md` is committed FIRST, BEFORE the re-run; (ii) the re-run output (`outputs/denominator_dual_method.json`) cites the frozen definition by **path + commit SHA**, and that cited commit is an **ancestor** of the re-run commit (`git merge-base --is-ancestor <frozen-def-commit> <rerun-commit>` exits 0); (iii) all three enumeration methods (M1 recorded-registry, M2 authored-intent, M3 identifier-census) apply the frozen counting unit + per-class predicates + the three exclusions (DP/HC/GPL/Pattern/Binding references; Cycle-16-own `Done #N`; naming-FP) identically before counting. A re-run that does not cite a prior-committed frozen definition, or that re-draws the definition to move the counts together, is REFUSED. **Verify:** `outputs/denominator_dual_method.json` carries `frozen_definition_path` + `frozen_definition_commit` non-empty + the ancestor check passes.

**Threshold 8 (NEW; ±5%-must-emerge / residual-disclosed / never-tuned — substrate §3.2).** Under the frozen definition, the cross-method spread is **MEASURED**. **±5% agreement, if it holds, EMERGES — it is never engineered by loosening or redrawing the definition.** If the three methods STILL diverge beyond ±5% under the frozen definition, that divergence is an HONEST FINDING, disclosed verbatim; the Build-Runner proceeds with the most-defensible single denominator (M1 definition-site count under the frozen unit) stated as **"100% of the discoverable population, residual R disclosed"** — the residual R is **disclosed, never zeroed, never estimated to zero**. A stable-because-tuned number is the operational-definition substitution one level up and is REFUSED. **Verify:** `outputs/denominator_dual_method.json` carries the three per-method counts [measured] + the measured spread + (if >±5%) the residual-disclosed close-claim string with no bare un-scoped "100%".

**KT cross-binding.** 6′ (single-reader-dominated V REFUSED) + threshold 7 (count-without-prior-frozen-def REFUSED) + threshold 8 (tuned-to-agree REFUSED) are the S21-HOLD root fixes encoded as HARD-FAIL acceptance bars. BE-R close-eval becomes **PASS-all (8 of 8)**: original thresholds 1-5 + threshold 6 superseded-by-6′ + new 7 + new 8. Reality-vs-intent baseline: at S21 the denominator is method-unstable (232/354/898 spread [measured]) and V is single-reader (E1 share 2.4% [measured]) — the refinement closes both; this is the gap, not a regression.

<!-- /gate:experimental_design §5.phase11.1-R stage_5_phase11_refinement -->

<!-- amendment_2026_05_30_s22_refinement_end -->

<!-- amendment_2026_05_30_s23_refinement_start -->
### §5.phase11.1-R S23 refinement — threshold 6″ (form-appropriate independent second reader) + grain-pin freeze-before-count binding + the disclosed-bound EXIT (Cycle-16-S23 RP refinement (a′)+(b′); additive-supersede-per-DP#42 of 6′ for the reader-independence content only)
...
<!-- source: docs/spec_class_frozen_definition.md §7 (a′) DesignDecision count-grain pin (the NEW freeze authored this same S23 RP fill) -->
<!-- source: outputs/validated_commitment_set.json S22 run [measured]: v_size 1818; E1∩E2 119 / E1_only 95 / E2_only 1604; e1_share 0.1177 / e2_solo_share 0.8823 / aggregate_jaccard 0.0655 -->
<!-- source: scripts/spec_extraction_pipeline.py extract_e1 (L270, rule_based_clause_parser) + extract_e2 (L343, llm_extraction:claude-haiku-4-5) + extraction_methods_distinct (L59, bare string compare) + spec_prose_window/_json_prose_for_key/_findings_block (L86-211 form logic) + T6P_*_FLOOR (L454-456) + assert_detector_input (L483-530) -->
<!-- source: cycle_16_s23_rp_refinement_dispatch_substrate §3 (form-appropriate reader) + §4 (anti-gaming guards) + §5 (the EXIT) -->
<!-- gate:experimental_design §5.phase11.1-R-S23 stage_5_phase11_refinement_2 required -->

**LOCKED-body discipline (Binding 7 + DP#42 strict).** §5.1-§5.recovery, §5.phase11, and §5.phase11.1-R (incl. threshold 6′/7/8) are LOCKED; this §5.phase11.1-R-S23 is ADDITIVE-APPEND only. Threshold 6″ below **additively supersedes (DP#42 non-destructive) the reader-independence CONTENT of 6′** — it re-authors what "distinct/independent readers" MEANS (replacing the bare `extraction_methods_distinct` string-compare as the acceptance content the Build-Runner implements). **The 6′ floors `T6P_CONTRIB_FLOOR=0.20 / T6P_SOLO_CEILING=0.80 / T6P_JACCARD_FLOOR=0.20` are UNCHANGED and stay fixed — 6″ does NOT touch them.** 6′'s composition-floor text is UNMODIFIED in place; 6″ adds the second-reader form-appropriateness + no-shared-blind-spot. Verify via `git diff` zero change on all pre-marker line ranges (HC #72).

**Why 6″ exists (the S22 HOLD-AGAIN finding 2).** S22 genuinely fixed E1 5.4× (its definition-site reader went from 2.4%→11.8% E1 contribution `[measured]`), and the strengthened 6′ guard CORRECTLY REFUSED the S22 V (validate-the-validator working): v_size 1818 = E1∩E2 119 + E1_only 95 + E2_only 1604; e1_share 0.1177 < 0.20 floor; e2_solo_share 0.8823 > 0.80 ceiling; aggregate Jaccard 0.0655 < 0.20 floor `[measured]`. The honest lesson: **a single rule-based reader cannot materially read prose-heavy specs** — on prose it extracts ~9× fewer atomic commitments than the LLM `[measured: 1604 E2-only vs 95 E1-only on the prose-dominated corpus]`. The fix is NOT the floor and NOT the guard; **the fix is giving the second reader a form it can actually read.** 6″ makes the second reader *method-appropriate*.

**Threshold 6″ (strengthened; re-authors reader-independence as the acceptance content — form-appropriate second reader + no shared blind spot; floors UNCHANGED).**

- **Form-classify each spec, then choose the form-appropriate second reader (decidable rule).** Compute, over the spec's resolved definition-site window (per `spec_prose_window`), the **prose fraction** `p = prose_tokens / (prose_tokens + structured_tokens)`, where structured tokens are JSON/code/markup tokens (keys, punctuation, brackets, `.ttl`/`.py`/`.json` syntax, table-cell delimiters) and prose tokens are natural-language word tokens outside structured spans. A spec is **structured** iff `p < 0.50` and **prose-heavy** iff `p ≥ 0.50` (the 0.50 boundary is the principled midpoint — a window is "prose-heavy" once natural-language obligation text is the majority of its content, which is exactly the regime where a rule-based clause-parser under-reads; it is NOT tuned to a target classification of the current corpus). The Build-Runner reports the per-spec `p` and the structured/prose split `[measured]`.
  - **Structured spec** (schema files, `state.json` blocks, registry rows): the **rule-based clause parser** (E1) is an adequate, genuinely-independent second reader against the LLM (E2) — structured prose has explicit, enumerable obligation anchors a deterministic parser reads competently.
  - **Prose-heavy spec** (agent `.md`, FINDINGS Layer-5 commitments): the second reader is a **diverse second LLM (E1′)** — a **different model** from E2's `claude-haiku-4-5` (e.g. a Sonnet/Opus-class model, NOT another Haiku-4-5 instance) AND a **genuinely different prompt** (a different extraction framing — e.g. obligation-by-obligation enumeration with a different schema/role frame — NOT E2's `E2_PROMPT` reworded). E1′ reads prose at a capability comparable to E2, removing the rule-vs-prose capability gap that floored the S22 V.
- **Re-author what "distinct/independent" MEANS (replaces the bare string-compare).** Two readers are independent iff **EITHER** (rule-based vs LLM) **OR** (LLM vs LLM with a **different model AND a different prompt**) — and in **NO** case the same mechanism twice, and in **NO** case the second reader tuned to reproduce the first. `extraction_methods_distinct` is upgraded from `method_e1 != method_e2` (bare string compare) to assert: (a) the two reader mechanisms differ; (b) when both are LLMs, their `model` identifiers differ AND their prompt identifiers/hashes differ; (c) the second reader was not parameterized/few-shot-seeded from the first reader's output.
- **The "no shared blind spot" test (operational).** The second reader MUST be **capable of finding commitments the first reader misses on the spec's own form** — so the intersection (E1∩E2 / E1′∩E2) is a *genuine agreement*, not a *subset-by-construction*. Operationalized: over the form-classified corpus, the second reader's **solo (reader-only) contribution MUST be > 0** on its appropriate form (a second reader that finds nothing the first missed is reading inside the first's blind spot, not independently) AND the aggregate composition MUST clear the UNCHANGED 6′ floors (e1/e1′ share ≥ 0.20, solo ≤ 0.80, Jaccard ≥ 0.20). A second reader whose extraction is a strict subset of the first's (zero solo) FAILS the no-shared-blind-spot test even if it nominally differs in mechanism.
- **Floors UNCHANGED and FIXED (the explicit refusal).** `T6P_CONTRIB_FLOOR=0.20`, `T6P_SOLO_CEILING=0.80`, `T6P_JACCARD_FLOOR=0.20` are NOT touched. **Lowering any floor to make V pass IS the operational-definition substitution recurring INSIDE the guard, and is REFUSED.** Tuning the diverse second LLM (E1′) to reproduce E2 (same model, E2's prompt reworded, or E2-output-seeded) IS the substitution recurring at the reader, and is REFUSED (it trips the upgraded independence assert). The bar is the *reader*, never the threshold.
- **Independence preserved across the upgrade (validate-the-validator).** 6″ does NOT relax 6′ threshold 4: the two readers stay genuinely independent. The form-appropriate choice (E1 for structured / E1′ diverse-LLM for prose) is selected to MATCH the spec's form so the second reader can read competently — NOT to mimic E2. A 6″ PASS achieved by collapsing the second reader into E2's mechanism is REFUSED.

**You re-author the CRITERION (spec) only.** The Build-Runner (next dispatch) implements the form-classifier (`spec_prose_window` form split), the diverse second LLM reader (E1′: different model + different prompt), the upgraded `extraction_methods_distinct`, and the no-shared-blind-spot check — re-running V under the form-appropriate reader. Per Binding 7 this is RP spec work reported to kc-53, NOT a Coach-direct gate edit. The Build-Runner does NOT touch `T6P_*` floors.

**Threshold 6″-FB (grain-pin freeze-before-count binding — substrate §4.1; strengthens threshold 7 for the S23 re-count).** The S23 denominator re-count is bound to the **grain-pin freeze**: (i) `docs/spec_class_frozen_definition.md` §7 (the DesignDecision count-grain pin) is committed FIRST, as a NEW freeze, BEFORE the Build-Runner re-counts; (ii) the re-count output (`outputs/denominator_dual_method.json`) cites the grain-pin freeze by **path + commit SHA**, and that cited commit is an **ancestor** of the re-count commit (`git merge-base --is-ancestor <grain-pin-commit> <recount-commit>` exits 0); (iii) all three methods (M1 / M2 / M3) map their raw artifacts to normalized **decision-identities** and dedup by the §7.2 canonical-identity hash BEFORE counting, per the §7.4 per-method reconciliation. **A re-count that does not cite a prior-committed grain-pin freeze, or that re-draws the grain / re-picks a sub-case / re-tunes the normalization to move 63/193/210/424 together, is REFUSED.** Verify: `outputs/denominator_dual_method.json` carries the grain-pin `frozen_definition_commit` (or a distinct `grain_pin_commit` field) non-empty + the ancestor check passes + the per-method decision-identity counts `[measured]`.

**Threshold 6″-EM (±5% must EMERGE under the pinned grain — substrate §4.2; strengthens threshold 8 for the S23 re-count).** Under the pinned grain (§7) the cross-method spread is **MEASURED**. ±5% agreement, if it holds, EMERGES — it is never engineered by re-drawing the grain. If the three methods STILL diverge beyond ±5% under the pinned grain, that divergence is an HONEST FINDING, disclosed verbatim, and the cycle takes the disclosed-bound EXIT below — NOT a third counting refinement. A stable-because-tuned number is REFUSED. Verify: `outputs/denominator_dual_method.json` carries the three per-method decision-identity counts `[measured]` + the measured spread + (if >±5%) the disclosed-bound close-claim string with no bare un-scoped "100%".

**The disclosed-bound EXIT (substrate §5 — the authorized stopping rule; binding acceptance language).** After a principled a′ (grain-pin) + b′ (form-appropriate second reader) and the Build-Runner re-run + Coach re-verification, the outcome is EXACTLY ONE of:
- **Trusted** — ±5% emerges across methods AND V passes 6″ (which includes the UNCHANGED 6′ composition floors) → record the trusted denominator + V; the cycle proceeds to BE-S/BE-T/Phase 12.
- **Disclosed-bound (the honest, already-authorized form of the bar — NOT a retreat)** — if they still don't, that is the **empirical finding that perfect cross-method agreement is not achievable for this corpus.** Adopt the **most-defensible disclosed-bound denominator** (M1′ under the frozen §1 unit + the §7 pinned grain — it counts authoritative decision-identity definition sites directly) + the **honestly-labeled extraction set**, and PROCEED toward the gap list. The close claim becomes **"100% of the disclosed-bound discoverable population, residual R disclosed."** **Do NOT loop a third counting refinement** — the exit is the authorized stopping rule.
- **Exit guard (so disclosed-bound is honesty, not surrender — ALL THREE BIND):** the disclosed-bound is legitimate ONLY with (i) a **principled grain frozen BEFORE seeing whether it was convenient** (§7 committed first, freeze-before-count); (ii) the residual **explicitly bounded and SMALL after a′ does its work** — the exit covers the irreducible remainder, **NOT the whole 63→424**; a large or unexplained residual is **another HOLD, not an exit**; (iii) the residual **carried forward into every downstream "100%" claim** (BE-T gap list, #53, FINDINGS) — disclosed, never zeroed.

**KT cross-binding.** 6″ (single-reader-on-prose REFUSED; floors fixed; second reader form-appropriate + no shared blind spot) + 6″-FB (re-count-without-prior-grain-pin-freeze REFUSED) + 6″-EM (grain-tuned-to-collapse REFUSED) + the disclosed-bound EXIT (third-counting-refinement-instead-of-exit REFUSED) are the S22-HOLD-AGAIN root fixes. BE-R S23 close-eval is PASS-all over thresholds 1-5 + 6′ (composition, floors unchanged) + 6″ (form-appropriate reader) + 7/6″-FB (grain-pin freeze-before-count) + 8/6″-EM (±5%-emerge-or-disclosed-bound-exit). Reality-vs-intent baseline: at S22 the DesignDecision grain is unpinned (63→424 spread `[measured]`) and V is single-reader-on-prose (e1_share 0.1177 `[measured]`); the S23 refinement closes both at the root; this is the gap, not a regression.

<!-- /gate:experimental_design §5.phase11.1-R-S23 stage_5_phase11_refinement_2 -->
